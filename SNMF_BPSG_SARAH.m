function [ Aout, xt, error, time ] = SNMF_BPSG_SARAH(y,sr,n_epochs, tau01,tau02, r, Ain, xin)
...
for k = 1 : n_epochs 
    ...
    if k>1
        ...
        L_A = power_method(A_old, pn)/sr;
        u  = 1/L_A;
        u = min(u_old, u);
        coeff = 3*(norm(A_old,'fro')^2+norm(xi_old,'fro')^2)+norm_y;
        grad01   =   A_old'*(A_old*xi_old - y)/sr;
        ...
        L_x = power_method(xi_old, pn)/sr;
        uy = 2/L_x;
        uy = min(uy_old, uy);
        grad02 = (A_old*xi_old - y)*xi_old'/sr;
    ...
    for i = 1 : sr  
        ...
        u = min(u_old, u);
        if k <= 1%10
            grad01   =  As'*(As*xi_old - ys); 
        else
            grad01   =  As'*(As*xi_old - ys) - Ass'*(Ass*xi_old_old - ys) + grad01;   
        end
        ...
        uy = min(uy_old, uy);
        if k <= 1%10
            grad02   =  (xi2*(A_old*xi2 - ys2)')' ; 
        else
            grad02   = (A_old*xi2 - ys2)*xi2' - (A_old_old*xii2 - ys2)*xii2' + grad02;
        end
        coeff = 3*(norm(A_old,'fro')^2+norm(xi_old,'fro')^2)+norm_y;   
        grad1 = grad01*u -coeff*xi_old;
        xi =-grad1; 
